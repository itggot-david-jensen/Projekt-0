...
function start(classlist=null){
    ...
    if (classlist != null) {
        ...
        for (i = classlist.length-1; 0 <= i; i--){
                        
            classlist[i].classList.remove("correct")
            classlist[i].classList.remove("wrong100")
            classlist[i].classList.remove("wrong66")
            classlist[i].classList.remove("wrong33")
            classlist[i].classList.remove("answered")
...
function reset(){
    ...
    start(document.getElementsByClassName("answered"))
...
function get_target(){
    ...
    if (alternatives[target_index] !== undefined){
        ...
        target_div.innerHTML = '<p>Epic Victory Royale!</p> <img src="img/special/nazir.gif">'
...
function is_correct(clicked_card){
    ...
    } else if (string_to_int(target["id"]) == string_to_int(clicked_card["id"]) || target["name"] == clicked_card["name"]){
    ...
    // console.log(removed_alternatives)
}
